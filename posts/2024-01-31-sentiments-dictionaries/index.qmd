...
```{r echo=FALSE}
library(emoGG)
happy_emojis <- emoji_search("happy") %>% filter(keyword == 'happy') %>% pull(code)
happy_emojis <- c(happy_emojis, c('1f4c8'))

sad_emojis <- emoji_search("sad") %>% filter(keyword == 'sad', emoji != "frowning_man") %>% pull(code)
sad_emojis <- c(sad_emojis, c('1f480', '2620'))

sinumoji <-
  tibble(x = seq(0, 2 * pi, by = 0.15),
         y = sin(x)) %>%
  rowwise() %>%
  mutate(emoji = ifelse(y > 0, sample(happy_emojis, size = 1), sample(sad_emojis, size = 1))) %>%
  ungroup()

ggplot(sinumoji, aes(x, y)) +
  geom_hline(yintercept = 0) +
  geom_emoji(emoji = sinumoji$emoji) +
  theme_minimal(base_family = "Comic Sans MS", base_size = 14) +
  theme(
  ...
  labs(
    y = '',
    x = 'Time',
    title = 'A Tale of Joy and Woe, equally Ze-roh',
    subtitle = 'Net Happiness: 0'
  )
